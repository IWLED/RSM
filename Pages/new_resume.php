...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        // CV - إضافة جميع الحقول
        $stmt = $conn->prepare("INSERT INTO cvs (user_id, name, template_name, is_published) VALUES (?,?,?,?)");
        $stmt->execute([
            $userId, 
            $_POST['name'],
        ...
        // Personal Info - إضافة جميع الحقول
        $stmt = $conn->prepare("
            INSERT INTO personal_info (
                cv_id, job_title, email, phone, address, summary, 
        ...
        foreach ($_POST['skill_name'] ?? [] as $i => $v) {
            ...
            // التأكد من أن المستوى هو أحد القيم المسموحة
        ...
        // يمكن إعادة التوجيه إلى صفحة العرض أو المعاينة
        // header("Location: view.php?id=" . $cvId);
        // exit;

    } catch (Exception $e) {
        $conn->rollBack();
        // للتطوير: عرض الخطأ الحقيقي
        die("فشل حفظ السيرة: " . $e->getMessage());
    }
}
...
<div class="content">
  <div class="cv-paper">
    ...
    <?php if(!empty($success)): ?>
      <div class="success">✅ تم حفظ السيرة الذاتية بنجاح</div>
    <?php endif; ?>
...
<div class="section active">
  ...
  <div class="field">
    ...
    <input name="name" placeholder="مثال: وليد بن محمد الحربي" required>
  </div>

  <div class="field">
    <label>نبذة مختصرة</label>
    <textarea name="summary" placeholder="نبذة قصيرة تعرّف عنك مهنيًا خلال 3–4 أسطر"></textarea>
  </div>
  ...
  <div class="grid two">
    ...
    <div class="field">
      <label>الجنسية</label>
      <input name="nationality" placeholder="سعودي">
    </div>
  </div>

  <div class="field">
    <label>العنوان</label>
    <input name="address" placeholder="المدينة – الدولة">
  </div>
  <!-- ===== PROFESSIONAL LINKS ===== -->
...
<div class="section">
  ...
  <button type="button" class="remove-btn" onclick="removeItem(this)">
    ...
    <!-- السطر الثاني: تاريخين -->
    <div class="grid two">
      <div class="field">
        <label>تاريخ البدء</label>
        <input type="date" name="start_date_edu[]">
      </div>

      <div class="field">
        <label>تاريخ الانتهاء</label>
        <input type="date" name="end_date_edu[]">
      </div>
    </div>
    ...
    <div class="field">
      ...
      <textarea name="description_edu[]" placeholder="تفاصيل إضافية عن الدراسة أو الإنجازات"></textarea>
    </div>
...
<div class="section">
  ...
  <button type="button" class="remove-btn" onclick="removeItem(this)">
    ...
    <!-- السطر الثاني: 3 عناصر -->
    <div class="grid three align-end">

      <div class="field">
        <label>تاريخ البدء</label>
        <input type="date" name="start_date_exp[]">
      </div>

      <div class="field end-date">
        <label>تاريخ الانتهاء</label>
        <input type="date" name="end_date_exp[]">
      </div>
...
<div class="current-job">
  <label>
    ...
    <div class="field">
      ...
      <textarea name="description_exp[]" placeholder="اذكر مهامك ومسؤولياتك وإنجازاتك"></textarea>
    </div>
...
<div class="section">
  ...
  <button type="button" class="remove-btn" onclick="removeItem(this)">
    ...
    <!-- السطر الأول: اسم المشروع + الرابط -->
    <div class="grid two">
      <div class="field">
